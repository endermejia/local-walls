...
describe('MentionLinkPipe', () => {
  ...
  it('should replace a single mention with an anchor tag', () => {
    ...
    expect(result).toBe(
      'Hello <a class="mention-link font-bold hover:underline cursor-pointer text-[var(--tui-text-action)]" href="/profile/123" data-id="123">@John Doe</a> !',
    );
  });
  ...
  it('should replace multiple mentions with anchor tags', () => {
    ...
    const result = pipe.transform(text);
    expect(result).toBe(
      '<a class="mention-link font-bold hover:underline cursor-pointer text-[var(--tui-text-action)]" href="/profile/1" data-id="1">@Alice</a> and <a class="mention-link font-bold hover:underline cursor-pointer text-[var(--tui-text-action)]" href="/profile/2" data-id="2">@Bob</a>',
    );
  });
  ...
  it('should handle mentions alongside special characters', () => {
    ...
    const result = pipe.transform(text);
    expect(result).toBe(
      'Hey <a class="mention-link font-bold hover:underline cursor-pointer text-[var(--tui-text-action)]" href="/profile/123" data-id="123">@User</a>, &quot;look&quot; at &lt;this&gt; &amp; that!',
    );
  });
});
