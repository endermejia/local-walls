...
export class MessagingService {
  ...
  async blockUserMessages(userIdToBlock: string): Promise<boolean> {
    if (!isPlatformBrowser(this.platformId)) return false;
    await this.supabase.whenReady();
    const myId = this.supabase.authUserId();
    if (!myId) return false;

    // Check if already blocked (upsert approach is cleaner if constraint exists, but let's be explicit)
    ...
    if (existing) {
      if (existing.block_messages) return true;
      const { error } = await this.supabase.client
        .from('user_blocks')
        .update({ block_messages: true })
        .eq('id', existing.id);
      return !error;
    } else {
      const { error } = await this.supabase.client.from('user_blocks').insert({
        blocker_id: myId,
  ...
  async unblockUserMessages(userIdToUnblock: string): Promise<boolean> {
    if (!isPlatformBrowser(this.platformId)) return false;
    await this.supabase.whenReady();
    const myId = this.supabase.authUserId();
    if (!myId) return false;
  ...
  async isUserBlockedByMe(userIdToCheck: string): Promise<boolean> {
    if (!isPlatformBrowser(this.platformId)) return false;
    await this.supabase.whenReady();
    const myId = this.supabase.authUserId();
    if (!myId) return false;

    const { data: block } = await this.supabase.client
      .from('user_blocks')
      .select('block_messages')
      .eq('blocker_id', myId)
      .eq('blocked_id', userIdToCheck)
      .single();

    return !!block?.block_messages;
  }
}
