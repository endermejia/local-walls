  ...
  async getTopoSignedUrl(
    ...
    if (error) {
      ...
      if (lastValid) {
        try {
          ...
          console.warn('[SupabaseService] Error parsing cached topo URL', e);
        }
      }
      return '';
    }
    ...
    if (version) {
      try {
        ...
        console.warn('[SupabaseService] Invalid URL for topo versioning', e);
      }
    }
    ...
    if (error) {
      ...
      if (isOffline) {
        try {
          ...
          console.warn('[SupabaseService] Error parsing cached ascent URL', e);
        }
      }
      return '';
    }
