...
describe('ROUTE_TABLE_SORTERS', () => {
  ...
  it('should sort by grade', () => {
    ...
    const rowA = createRow({
      _ref: { grade: VERTICAL_LIFE_GRADES.G5a } as RouteWithExtras,
    });
    const rowB = createRow({
      _ref: { grade: VERTICAL_LIFE_GRADES.G8a } as RouteWithExtras,
    });
  ...
  it('should sort by topo name (joined), falling back to route name', () => {
    ...
    const rowA = createRow({
      topos: [{ id: 1, name: 'Alpha Topo' }],
      route: 'Z Route',
    });
    const rowB = createRow({
      topos: [{ id: 2, name: 'Beta Topo' }],
      route: 'A Route',
    });
