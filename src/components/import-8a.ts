...
export class Import8aComponent {
  ...
  async onImport(): Promise<void> {
    ...
    try {
      // 1. Get all unique names of areas and crags
      const uniqueAreaNames = [...new Set(ascents.map((a) => a.location_name))];

      // 2. Find all existing routes in the entire DB.
      // We look for both the base slug and the "unified" slug (with the sector) to avoid collisions.
      ...
      // 3. Classify ascents: those that already have a route vs those that need to create it
      for (const a of ascents) {
        ...
        if (routeId) {
          ...
          // Route does not exist and is admin -> needs to create route
      ...
      if (isAdmin && ascentsToCreateRoutes.length > 0) {
        ...
        if (routesUpsertData.length > 0) {
          ...
          // Safely update routeMap with the new routes
          if (newRoutes) {
            for (const a of ascentsToCreateRoutes) {
              const areaId = areaMap.get(slugify(a.location_name));
              const cragId = cragMap.get(`${areaId}|${slugify(a.sector_name)}`);
