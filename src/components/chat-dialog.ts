  ...
  template: `
    <div class="flex flex-col h-[70dvh] min-h-[500px] -m-4">
      @if (selectedRoom(); as room) {
        <!-- Messages View -->
        <div class="flex items-center p-2 border-b border-[var(--tui-border-normal)] gap-2">
            <button
                tuiIconButton
                type="button"
            ...
            ></button>
            <tui-avatar
                [src]="supabase.buildAvatarUrl(room.participant.avatar) | tuiFallbackSrc: '@tui.user' | async"
                size="s"
            />
            <span class="font-bold truncate text-sm">{{ room.participant.name }}</span>
        </div>

        <tui-scrollbar #scrollbar class="grow min-h-0" (scroll)="onScroll($event)">
            <div class="flex flex-col gap-4 p-4">
                @if (hasMore() && !loadingMessages()) {
                    <div class="flex justify-center">
                        <button tuiButton appearance="flat-grayscale" size="xs" (click)="loadMoreMessages()">
                ...
                @for (msg of messages(); track msg.id) {
                    ...
                    <div class="flex" [class.justify-end]="isMe">
                        ...
                        >
                            <p class="whitespace-pre-wrap break-words leading-tight">{{ msg.text }}</p>
                            <div class="text-[10px] opacity-60 text-right mt-1 flex items-center justify-end gap-1">
                                {{ msg.created_at | date: 'HH:mm' }}
                                @if (isMe) {
                                    @if (msg.read_at) {
                                        <tui-icon icon="@tui.check-check" class="!w-3 !h-3 text-blue-400" />
                                    } @else {
        ...
        <div class="p-4 border-t border-[var(--tui-border-normal)]">
            <tui-textfield class="w-full" tuiTextfieldSize="m" [tuiTextfieldCleaner]="false">
                <label tuiLabel for="new-message">{{ 'labels.typeMessage' | translate }}</label>
                <input
                    tuiTextfield
                    id="new-message"
                    autocomplete="off"
                    ...
                    >
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-center gap-2">
                                <span class="font-bold truncate">{{ room.participant.name }}</span>
...
export class ChatDialogComponent {
  ...
  protected readonly selectedRoom = signal<ChatRoomWithParticipant | null>(null);
  ...
  protected readonly loadingRooms = computed(() => this.roomsResource.isLoading());
  ...
  protected readonly messagesResource = resource({
    ...
    loader: ({ params }) => {
        ...
        return this.messagingService.getMessages(params.roomId, this.limit, params.offset);
    }
  });

  protected readonly accumulatedMessages = signal<ChatMessageDto[]>([]);
  protected readonly messages = computed(() => [...this.accumulatedMessages()].reverse());
  protected readonly loadingMessages = computed(() => this.messagesResource.isLoading());
