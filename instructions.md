# Database Updates for User Blocking

Run the following SQL commands in your Supabase SQL Editor to implement the blocking feature.
...
Modify the policy to hide messages from blocked users (mutual visibility check).

```sql
-- Drop the existing policy (name from supabase-rls.json)
...
using (
  ...
  and not exists (
    ...
    where (blocker_id = auth.uid() and blocked_id = chat_messages.sender_id)
       or (blocker_id = chat_messages.sender_id and blocked_id = auth.uid())
  )
);
```
...
Modify the policy to hide ascents from blocked users (mutual visibility check).
...
using (
  ...
  (
    ...
    and not exists (
      select 1 from user_blocks
      where (blocker_id = auth.uid() and blocked_id = route_ascents.user_id)
         or (blocker_id = route_ascents.user_id and blocked_id = auth.uid())
    )
  )
);
```
